...
def render_sandbox_controls(theme: Mapping[str, Any], mode: str, params: Mapping[str, Any]) -> html.Div:
    ...
    # Order Type is the same question in all three modes, so it sits below the
    # mode-specific knobs rather than inside any of them. Exit Handling is
    # omitted for accumulation, which has no exits to hand anywhere.
    controls.append(wrap('Order type', 'how a signal is worked', dcc.Dropdown(
        id={'type': 'exec-param', 'mode': mode, 'name': 'order_type'},
        options=[
            {'label': 'Market — fills at the close', 'value': 'market'},
            {'label': 'Limit — waits for a better price', 'value': 'limit'},
            {'label': 'Stop — waits for confirmation', 'value': 'stop'},
            {'label': 'Stop-limit — capped slippage', 'value': 'stop_limit'},
        ],
        value=params.get('order_type', 'market'),
        clearable=False, className='dark-dropdown')))
    controls.append(wrap('Order offset %', 'how far from the close it rests', dcc.Slider(
        id={'type': 'exec-param', 'mode': mode, 'name': 'order_offset_pct'},
        min=0, max=6, step=0.5, value=params.get('order_offset_pct', 2.0),
        marks=None, tooltip={'placement': 'bottom', 'always_visible': True})))

    if mode != 'accumulation':
        controls.append(wrap('Exit handling', 'what works the way out', dcc.Dropdown(
            id={'type': 'exec-param', 'mode': mode, 'name': 'exit_order_mode'},
            options=[
                {'label': 'Close check — the default', 'value': 'close'},
                {'label': 'Trailing stop order', 'value': 'stop_order'},
                {'label': 'OCO bracket', 'value': 'bracket'},
            ],
            value=params.get('exit_order_mode', 'close'),
            clearable=False, className='dark-dropdown')))
...
def render_sandbox(
    ...
    if revealed:
        body.extend([
            ...
            stats,
            # Generated by the same run as the ledger below it, so it can never
            # describe an order model the tape did not actually execute.
            html.Div(
                order_model_summary(mode, **merged),
                className='sfa-exec-tape-note',
                style={'color': theme['text_tertiary']},
            ),
