...
(function () {
    ...
    /** Render the floating O/H/L/C readout. `bar` is null when the cursor leaves. */
    function updateLegend(bar, volume) {
        ...
        var html = '<span class="sfa-chart-legend-symbol">' + (meta.ticker || '') + '</span>';
        html += '<span class="sfa-chart-legend-interval">' + (meta.interval || '').toUpperCase() + '</span>';
    ...
    function ensureChart(payload) {
        ...
        state.chart.subscribeCrosshairMove(function (param) {
            ...
            updateLegend(bar, vol ? vol.value : null);
        });
    ...
    var api = {
        ...
        apply: function (payload) {
            if (!payload || !payload.candles) { return false; }
            var meta = payload.meta || {};
