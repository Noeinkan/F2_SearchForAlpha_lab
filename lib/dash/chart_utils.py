...
def create_chart(df: pd.DataFrame, config: Dict) -> go.Figure:
    ...
    try:
        ...
        update_layout(fig, plot_count, row_heights, config['show_legend'])
...
def add_rsi(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], name="RSI", line=dict(color='orange', width=1)), row=row, col=col)
...
def add_cci(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['CCI'], name="CCI", line=dict(color='purple', width=1)), row=row, col=col)
...
def add_macd(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['MACD'], name="MACD", line=dict(color='blue', width=1)), row=row, col=col)
    fig.add_trace(go.Scatter(x=df.index, y=df['MACD_Signal'], name="Signal", line=dict(color='orange', width=1)), row=row, col=col)
    fig.add_bar(x=df.index, y=df['MACD_Histogram'], name="Histogram", marker_color='gray', row=row, col=col)

def add_adx(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['ADX'], name="ADX", line=dict(color='brown', width=1)), row=row, col=col)
...
def add_atr(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['ATR'], name="ATR", line=dict(color='teal', width=1)), row=row, col=col)

def add_obv(fig: go.Figure, df: pd.DataFrame, row: int, col: int, config: Dict) -> None:
    fig.add_trace(go.Scatter(x=df.index, y=df['OBV'], name="OBV", line=dict(color='magenta', width=1)), row=row, col=col)
...
def update_layout(fig: go.Figure, plot_count: int, row_heights: List[int], show_legend: bool) -> None:
