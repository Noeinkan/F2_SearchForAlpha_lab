...
- **Configuration** — `config/strategy_config.yaml` (per-indicator params `strategies.sma / ema / rsi / bollinger_bands / cci / stoch / macd / vwap`, `signal_registry` controls, `backtest.{initial_capital, percent_of_portfolio, delay}`, and the `agent_strategies:` block with 14 named bundles).
...
- **Risk-bounded paper execution** — `lib/live/guards.py` evaluates four guards each bar and each heartbeat (`daily_loss`, `position_size`, `broker_disconnected`, `clock_drift`) and the runner cancels open orders, disconnects and posts an alert to `SFA_ALERT_WEBHOOK` on the first trigger; it reconnects across the IB Gateway daily restart (`lib/live/connection.py`); order-size caps `max_order_quantity` and `max_order_notional` are configurable in `config/agent.yaml`. Orders follow the strategy's backtested order model (market, limit, stop, stop-limit with `gtc`/`day`/`ioc` and bar expiry, `lib/live/execution.py`), and `sfa run` refuses `live_params` whose exits, sizing or signal gating the runner does not reproduce.
...
- **Deployment** — `scripts/run_dashboard_latest.ps1` (Windows launcher with `KillAll`/`NoOpen`/`Foreground`), `deploy.ps1` (server deploy with `SkipPipInstall` / `SkipRestartDashboard` deploy parameters and a rollback path), `sfa.cmd`. README explicitly warns that the Werkzeug reloader is unreliable on Windows and defaults it off.
...
- **Fundamentals cover only U.S. SEC** — non-U.S. tickers fall back to yfinance annual and quarterly statements. U.S. quarterly statements come from SEC filings, with derived fourth quarters, so a derived Q4 EPS is approximate.
- **Indicator set is fixed** — exactly eight indicators (RSI, BB, MACD, EMA, SMA, CCI, STOCH, VWAP) plus ADX/ATR/OBV indicators; no ML-built signals, no order-book microstructure, no sentiment NLP.
