...
- **Ticker universe** — regenerate via `scripts/generate_ticker_universe.py` writing to `config/tickers_universe.csv`; the loader falls back to a minimal bootstrap list and `resolve_universe_ticker()` in `lib/data_processing.py` if the CSV is absent.
- **Theming** — palettes are declared under `THEMES` in `lib/dash/dash_config.py`; the default is `bloomberg`. The `theme` key in `config/strategy_config.yaml` overrides the default at boot.
- **Test surface** — `lib/tests/` holds 29 test files grouped by area (strategy, signals, backtest, optimisation, walkforward, promotion, CLI contracts, runner safety, broker mock, dashboard startup/routing/data, fundamentals, flow scanner). Run with `rtk python -m pytest lib/tests/ -q`.
