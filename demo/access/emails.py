"""Email addresses: validation, the canonical form a trial is keyed on, blocklist.

A trial belongs to a mailbox, not to a spelling of it. ``Jane.Doe+demo@gmail.com``
and ``janedoe@gmail.com`` land in the same inbox, so they share one trial:
``canonical`` lower-cases, drops a ``+tag`` everywhere, and drops the dots Gmail
ignores. Mail is still sent to the address as typed.

The blocklist holds the throwaway-inbox services that make "one trial per
...
_SHAPE = re.compile(r"^[^@\s<>()\[\],;:\"]{1,64}@[A-Za-z0-9](?:[A-Za-z0-9-]{0,62}[A-Za-z0-9])?(?:\.[A-Za-z0-9](?:[A-Za-z0-9-]{0,62}[A-Za-z0-9])?)*\.[A-Za-z]{2,63}$")
