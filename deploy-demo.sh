#!/usr/bin/env bash
# deploy-demo.sh — publish the public demo (slug alpha) to the Hetzner edge.
#
# Not the workspace deploy: deploy.ps1 is that. This one ships only what
# .deploy/ describes, i.e. demo/server.py behind alpha.demos.noeinsolutions.com.
#
# A thin wrapper so the repo has one obvious entry point:
#
#   bash deploy-demo.sh              deploy the ref in .deploy/site.json
#   bash deploy-demo.sh --dry-run    print what would happen, change nothing
#   bash deploy-demo.sh --rollback   restore the last good commit
#
